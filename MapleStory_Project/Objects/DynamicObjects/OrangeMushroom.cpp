...
namespace
{
constexpr DirectX::SimpleMath::Vector2 groundSize = { 10.0f, 60.0f };
constexpr DirectX::SimpleMath::Vector2 sensorSize = { 60.0f, 60.0f };
}


OrangeMushroom::OrangeMushroom(DirectX::SimpleMath::Vector2 position, DirectX::SimpleMath::Vector2 scale, float rotation, const std::wstring& texturePath, BodyType bodyType, const std::string& name)
{
	...
	monsterCol->SetColliderScale(DirectX::SimpleMath::Vector2(groundSize));
	// 충돌 레이어 : Monster
	...
	monsterSensorCol->SetColliderScale(DirectX::SimpleMath::Vector2(sensorSize));
	...
	// XML 애니메이션 데이터 로드
	monsterAni->Load(L"_Animations/OrangeMushroom.xml");
