...
void CameraController::Update()
{
    ...
    // ImGui UI 위에서 마우스 입력이 사용 중이면
    if (ImGuiManager::GetInstance().WantCaptureMouse())
    {
        // UI 스크롤 시 카메라 줌에 입력이 남지 않도록 휠 초기화
        InputManager::GetInstance().ResetMouseWheel();
        // UI 위에서 입력이 차단되더라도 다음 프레임 드래그 계산이 튀지 않도록 마우스 위치 갱신
        prevMousePos = mouse;
        return;
    }
